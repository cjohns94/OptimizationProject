...
function [robot_opt,exitflag, output,info] = pso_dhOptimizer(robot, poses)
    tic
    q_curr = zeros(1,6);
    DOF = robot.n;
% 
...
%     end
    ...
    alpha_lb = ones(DOF,1)*-1*pi;
    offset_lb = ones(DOF,1)*-1*pi;    
    lb= [d_lb; a_lb; alpha_lb; offset_lb]; 
    
    %Column vectors for lower bounds
    d_ub = ones(DOF,1)*0.5;
    a_ub = ones(DOF,1)*0.5;
    alpha_ub = ones(DOF,1)*1*pi;
    offset_ub = ones(DOF,1)*1*pi;    
...
%         'Diagnostics', 'on');
    ...
    options = psooptimset('PopulationSize', 20, 'PlotFcns', {@psoplotbestf,@psoplotswarmsurf}, 'PopInitRange', [lb.';ub.'], 'Display', 'diagnose');
    [x_opt,fval,exitflag,output,info.population,info.scores] = pso(@(x) pose_error(x, poses), length(x0), A,b,Aeq, beq, lb, ub, @constraints, options);
